...
def gen_basic_neuron(graph: tf.Graph, x: tf.Tensor, w: tf.Tensor, b: tf.Tensor) -> tf.Tensor:
...
def gen_basic_loss(graph: tf.Graph, y: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
    """Generates a basic loss function and returns a pointer to it"""

    with graph.as_default():
        with tf.name_scope("loss"):
            loss = tf.reduce_sum(((y - tf.squeeze(y_pred))**2))
